...
#
# in the following code, I use the trained model by mxnet_train_resnext_singlelabelmultitask.py to do the forward pass
#
# reference:
...
def load_classifier(prefix, index, batch_size, img_width, img_height):
    ...
    label_shapes = [('softmax_multitask1_label', (batch_size,)), ('softmax_multitask2_label', (batch_size,))]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model',        type = str,   default = '/root/mxnet_dpn/models/mnist224_resnext50_SLMT/resnext50', required = True)
    ...
    for image_name in image_list:
        ...
        if DEBUG:
            print (image_path, pred2)
            gt_label = int(image_path.split('/')[-2])
            if (gt_label > MULTITASK_LABEL2_THRES) == pred2[0]:
                test_count2 += 1
            print test_count2

    print "task1 acc: ", test_count1 * 1.0 / len(image_list)
    print "task2 acc: ", test_count2 * 1.0 / len(image_list)